...
The content is aligned with the existing Huachuang (华创系) knowledge graph sample data,
...
def set_cell_font(cell, text, bold=False, size=10.5):
    """Helper: set cell text with font formatting."""
    cell.text = ""
...
def build_document():
    ...
    run = subtitle.add_run(f"报告日期：2025年5月28日　　　密级：内部机密")
    ...
    doc.add_paragraph(
        "本报告基于公开市场信息与监管披露文件，对华创控股集团有限公司（以下简称"华创集团"）及其关联企业"
        "（合称"华创系"）2024年以来的重大风险事件进行综合分析。华创系涉及企业主体包括华创控股集团"
    ...
    for i, (name, role, companies, risk) in enumerate(persons):
        row = person_table.rows[i + 1]
        set_cell_font(row.cells[0], name, size=10)
        set_cell_font(row.cells[1], role, size=10)
        set_cell_font(row.cells[2], companies, size=10)
    ...
    # --- Event 1 ---
    doc.add_paragraph()
    event1_title = doc.add_paragraph()
    run = event1_title.add_run("事件一：华创集团被列入被执行人（执行案号：(2024)京01执字第01234号）")
    run.font.size = Pt(12)
    run.font.name = "黑体"
    run.bold = True
    ...
    # --- Event 2 ---
    doc.add_paragraph()
    event2_title = doc.add_paragraph()
    run = event2_title.add_run("事件二：海通金融涉嫌非法集资（案件编号：公经侦(2024)第05678号）")
    run.font.size = Pt(12)
    run.font.name = "黑体"
    run.bold = True
    ...
    doc.add_paragraph(
        ...
        "侦查。初步查明，该公司自2022年起以"私募股权投资基金"名义，通过鑫达投资管理有限公司"
    ...
    # --- Event 3 ---
    doc.add_paragraph()
    event3_title = doc.add_paragraph()
    run = event3_title.add_run("事件三：鑫达投资股权冻结（裁定书编号：(2024)沪74民初字第00345号）")
    run.font.size = Pt(12)
    run.font.name = "黑体"
    run.bold = True
    ...
    # --- Event 4 ---
    doc.add_paragraph()
    event4_title = doc.add_paragraph()
    run = event4_title.add_run("事件四：证监会立案调查华创系（调查通知编号：证监调查字(2024)第00128号）")
    run.font.size = Pt(12)
    run.font.name = "黑体"
    run.bold = True
    ...
    # --- Event 5 ---
    doc.add_paragraph()
    event5_title = doc.add_paragraph()
    run = event5_title.add_run("事件五：华创地产债券违约（债券代码：20华创01，ISIN：CN01200001234）")
    run.font.size = Pt(12)
    run.font.name = "黑体"
    run.bold = True
    ...
    doc.add_paragraph(
        ...
        "受托管理人：中信信托有限责任公司\n"
        "违约金额：本息合计8.5亿元人民币\n"
        "事件概述：华创地产股份有限公司发行的"20华创01"公司债券未能按期兑付本息，构成"
    ...
    doc.add_paragraph(
        ...
        "治理的透明度，增加了风险传导的隐蔽性。该交叉持股闭环已被识别为"股权穿透异常"风险特征"
        "（特征编号F001）。"
    )
    doc.add_paragraph(
        "（三）连环担保网络\n"
        ...
        "超过50%，已被识别为"担保链风险"特征（特征编号F002）。一旦任一节点发生违约，担保链"
        "将产生多米诺骨牌效应。"
    )
    doc.add_paragraph(
        "（四）高管交叉任职\n"
    ...
    doc.add_paragraph(
        "路径一（司法 → 刑事 → 监管 → 信用）：\n"
        "华创集团被列入被执行人（2024.03.15）→ 子公司银行账户冻结、流动枯竭 → 华创地产"
        "债券违约（2025.01.18）→ 华创地产信用评级下调（2025.02.01）。\n"
        "该路径展示了司法执行如何通过资金链传导最终演变为信用风险事件。"
    )
    doc.add_paragraph(
        "路径二（刑事 → 司法 → 监管）：\n"
        "海通金融非法集资案发（2024.06.20）→ 鑫达投资股权冻结（2024.09.10）→ 证监会立案"
        "调查华创系三家公司（2024.12.05）→ 华创控股信披违规处罚（2025.03.22）。\n"
        "该路径展示了刑事案件如何触发监管调查、进而引发行政处罚的连锁反应。"
    )
    doc.add_paragraph(
        "以上两条路径最终汇合，共同作用于整个华创系，形成"司法执行 → 刑事立案 → 股权冻结 → "
        "行政调查 → 债务违约 → 评级下调 → 行政处罚"的完整风险传导链条。"
    )
    ...
    doc.add_paragraph(
        ...
        "3. 建议证监会向华创控股、华创地产发送监管问询函，要求限期补充披露关联交易和对外担保"
        "事项；\n"
