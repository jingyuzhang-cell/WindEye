...
from report_settings import CLUSTERING_OUTPUT_DIR, GRAPH_DATA_FILE, ensure_output_dirs
...
def parse_args():
    ...
    parser.add_argument(
        "--input", type=str, default=str(GRAPH_DATA_FILE),
        help="知识图谱输入文件路径"
    )
    parser.add_argument(
        "--output", type=str, default=str(CLUSTERING_OUTPUT_DIR),
        help="聚类结果输出目录"
    )
    parser.add_argument(
