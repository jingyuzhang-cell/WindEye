...
MODEL_NAME = DEEPSEEK_MODEL  # DeepSeek模型名称
...
def call_deepseek_api(prompt: str, system_prompt: str) -> Optional[str]:
    ...
    """
    if not API_KEY:
        print("  ⚠ 未设置 DEEPSEEK_API_KEY/LLM_API_KEY，使用模拟模式")
        return None
...
def main():
    ...
    # ---------- 配置检查 ----------
    if not API_KEY:
        print("\n⚠ 警告: 未检测到 DEEPSEEK_API_KEY 或 LLM_API_KEY")
        print("  将使用回退方案（基于图谱数据自动生成基础报告）")
        print("  若要使用LLM生成高质量报告，请在 backend/.env 中配置对应密钥\n")
