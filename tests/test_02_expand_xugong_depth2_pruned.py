...
def main() -> int:
    ...
    if not node_id:
        ...
        save_result("02_expand_xugong_depth2_pruned.json", payload)
        print_row(payload)
        return 1
    ...
    result = call_json(base_url, "POST", api, body, timeout=180)
    ...
    graph_ok = bool(
        ...
        and pruning.get("policy") == "degree_aware"
        and pruning.get("terminalHubCount", 0) > 0
    )
    payload = wrap_result(
        ...
        extra={
            ...
            "truncatedBy": summary.get("truncatedBy"),
        },
    )
    save_result("02_expand_xugong_depth2_pruned.json", payload)
    if graph_payload:
        save_result("02_expand_xugong_depth2_pruned_graph.json", graph_payload)
    print_row(payload)
